# Write a program that uses print statements to print the following recipe into the Output console. 
# The text to print is already there, you just need to make it into code. 
# Your code should print all five lines exactly the same as the example output below. 
# Make sure that you don't change any of these existing text as everything, punctuation and 
# casing all need to match!

print('Mix 5800g of Flour, 10g Yeast and 300ml Water in a bowl.')
print('Knead the dough for 10 minutes.')
print('Add 3g of Salt.')
print('Leave to rise for 2 hours.')
print('Bake at 200 degrees C for 30 minutes.')
print('Hello '+input('What is your name?:') +'!')
print('Length of your input is:'+str(len(input('Enter the input for which you want to calculate the length:'))))